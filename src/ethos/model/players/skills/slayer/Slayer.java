	...
	public void createNewTask(int masterId) {
		SlayerMaster.get(masterId).ifPresent(m -> {
			if(player.getName().equalsIgnoreCase("Wolf")) {
				player.sendMessage("m.getLevel() = " + m.getLevel());
				player.sendMessage("player.calculateCombatLevel() = " + player.calculateCombatLevel());
			}
			if (player.calculateCombatLevel() < m.getLevel()) {
				player.getPA().removeAllWindows();
				player.sendMessage("You need a Combat Level of <col=ff0000><shad=000000>" + m.getLevel() + "</shad></col> to use this Slayer Master");
				return;
			}
