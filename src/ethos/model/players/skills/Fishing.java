			...
			if (npcId == data[i][0]) {
				...
				CycleEventHandler.getSingleton().addEvent(player, new CycleEvent() {
					...
					public void execute(CycleEventContainer container) {
						...
						if (player.playerSkillProp[10][1] > 0) {
							...
							if (Boundary.isIn(player, Boundary.RESOURCE_AREA)) {
								if (Misc.random(20) == 5) {
									...
									player.sendMessage("You received " + randomAmount + " coins while fishing!");
									player.getItems().addItem(995, randomAmount);
								}
							}
						...
						if (player.caughtFish >= Config.MaxFish) {
							player.caughtFish = 0;
							player.sendMessage("<shad=0000ff>You have caught " + Config.MaxFish + " fish, please start fishing again.</shad>");
							container.stop();
						}
					}
