...
public class AttackNPC {
	...
	public static void delayedHit(final Player c, final int i, Damage damage) {
		...
		if (damage.getAmount() > npc.getHealth().getAmount()) {
			damage.setAmount(npc.getHealth().getAmount());
		}
		...
		if (NPCHandler.npcs[i] != null) {
			...
			RaidRewards.givePoints(c, npc.npcType, damage.getAmount());
...
//				break;
			...
			case 7573:
				c.getRaids().increaseDamage(RaidMonsters.LIZARD_SHAMAN, damage.getAmount());
				break;
				
			case 7585:
				c.getRaids().increaseDamage(RaidMonsters.ICE_DEMON, damage.getAmount());
				break;
				
			case 7604: //Skeletal mystic
			case 7605: //Skeletal mystic
			case 7606: //Skeletal mystic
				//c.getRaids().increaseDamage(RaidMonsters.SKELETAL_MYSTIC, damage.getAmount());
				break;
	...
	public static boolean isAttackable(Player player, int i) {
		if (!NPCHandler.npcs[i].inMulti() && NPCHandler.npcs[i].npcType != 5890) {
	...
	public static void attackNpc(Player c, int i) {
		...
		if (npc.npcType == 7585 && RaidMonsters.ICE_DEMON.isInBounds(c)) {
			Player raidLeader = c.getRaids().getRaidLeader();
		...
		if (npc.npcType == 7554 && RaidMonsters.GREAT_OLM.isInBounds(c)) {
			if (!c.getRaids().handsDead()) {
				c.sendMessage("Your attacks will be ineffective until his hands have been dealt with.");
				c.getCombat().resetPlayerAttack();
				return;
			}
		}
		...
		/*if (NPCHandler.npcs[i].npcType == 499) {
			if (!c.getSlayer().getTask().isPresent()) {
				return;
			}
			if (!c.getSlayer().getTask().isPresent() && !c.getSlayer().getTask().get().getPrimaryName().equals("smoke devil") && !c.getSlayer().getTask().get().getPrimaryName().equals("thermonuclear smoke devil")) {
					c.sendMessage("You are not on a Smoke Devil task.");
					c.getCombat().resetPlayerAttack();
					return;
			}
		}*/
		...
		if (NPCHandler.npcs[i] != null) {
			...
			if (NPCHandler.npcs[i].npcType == 5890 && NPCHandler.npcs[i].underAttackBy > 0 && NPCHandler.npcs[i].underAttackBy != c.getIndex() && NPCHandler.npcs[i].inMulti()) {
				c.npcIndex = 0;
				c.sendMessage("This monster is already in combat.");
				return;
			}
			if (NPCHandler.npcs[i].underAttackBy > 0 && NPCHandler.npcs[i].underAttackBy != c.getIndex() && !NPCHandler.npcs[i].inMulti()) {
				c.npcIndex = 0;
				c.sendMessage("This monster is already in combat.");
				return;
			}
			...
			if (c.attackTimer <= 0) {
				...
				/*if ((!c.goodDistance(c.getX(), c.getY(), NPCHandler.npcs[i].getX(), NPCHandler.npcs[i].getY(), 2)
						&& (c.getCombat().usingHally() && !c.usingOtherRangeWeapons && !c.usingBow && !c.usingMagic))
				...
				if (c.usingSpecial && !c.usingMagic) {
					...
					if (special.getRequiredCost() > c.specAmount) {
						c.sendMessage("<col=CC0000>You don't have the required special energy to use this attack</col>.");
				...
				c.lastWeaponUsed = c.playerEquipment[c.playerWeapon];
				c.lastArrowUsed = c.playerEquipment[c.playerArrows];
				if (!c.usingBow && !c.usingMagic && !c.usingOtherRangeWeapons && !c.usingBallista) { // melee
																					// hit
																					// delay
