...
public class ObjectOptionOne {
	...
	public static void handleOption(final Player c, int objectType, int obX, int obY) {
		...
		if (c.getRaids().handleObjects(objectType, obX, obY)) {
			return;
		}
		...
		case 29777:
			...
			if (c.clan == null || !c.clan.isFounder(c.getName())) {
				c.sendMessage("You're not in a clan that you own, and can not pass the door.");
				return;
			}
			
			int memberCount = c.clan.activeMembers.size();
			
			if (memberCount < 1) {
				c.sendMessage("You don't have enough people in your clan to start a raid.");
				return;
			}
			
			if (memberCount > 22) {
				c.sendMessage("Your clan exceeds the max limit of 22 players in Raids.");
				return;
			}
			...
			for (String username : c.clan.activeMembers) {
				...
				if (p == null || p.disconnected || !p.isActive || p.getIndex() == c.getIndex()) {
					continue;
				}
				
				p.getPA().movePlayer(Raids.START_X, Raids.START_Y, c.getRaids().raidHeight);
				p.sendMessage("You have been teleport to a raid by "+c.getName());
			}
			break;
		...
		case 29150:
			...
			int interfaceId = c.playerMagicBook == 0 ? 28062 : (c.playerMagicBook == 1 ? 29999 : 1151);
			...
			return;
		case 29241: // Ornate Rejuvenation Pool (at home)
			if (c.specRestore > 0) {
			...
			c.specRestore = 120;
			if (c.getRights().isOrInherits(Right.DONATOR) || c.getRights().isOrInherits(Right.ADMINISTRATOR)) {
				c.specAmount = 10.0;
				c.getItems().addSpecialBar(c.playerEquipment[c.playerWeapon]);
				c.sendMessage("You have recharged your HP, Prayer, and Spec");
			} else {
				c.sendMessage("You have recharged your HP and Prayer");
			}
			if (!c.getRights().isOrInherits(Right.DONATOR) && !c.getRights().isOrInherits(Right.ADMINISTRATOR)) {
				c.sendMessage("<img=6> Did you know? If you are a Donator you'll restore your special attack fully here!");			
			}
			break;
		case 30364:
			int mininglevel = c.playerLevel[c.playerMining];
			if(mininglevel >= Config.MiningGuildReq) {
				if(c.getPosition().getY() >= 9757) {
					c.getPA().movePlayer(3046, 9756);
					c.sendMessage("<shad=00ff00>Now entering the Mining Guild!</shad>");
				} else {
					c.getPA().movePlayer(3046, 9757);
					c.sendMessage("<shad=ff0000>Now leaving the Mining Guild!</shad>");
				}
			} else {
				c.sendMessage("You need to have at least Level " + Config.MiningGuildReq + " Mining to enter here!");
			}
			break;
		case 6150:
		...
		break;
		/*case 29747: //Ice Demon Brazziers
			if (c.getItems().playerHasItem(20799, 1)) {
				Server.getGlobalObjects().replace(new GlobalObject(29747, obX, obY, c.heightLevel, 0, 10, 50, -1), new GlobalObject(29748, obX, obY, c.heightLevel, 0, 10, 0, -1));
				c.getItems().deleteItem(20799, 1);
			} else {
				c.sendMessage("You need some kindling to light this brazier!");
			}
			break;
		case 29748:
			if (c.getItems().playerHasItem(20799, 1)) {
		...
		case 7811:
			...
			break;
		case 4150:
		c.getPA().spellTeleport(2855, 3543, 0);
		case 23115://from bobs
		c.getPA().spellTeleport(1644, 3673, 0);
		break;
		case 10251:
		...
		case 20877:
			AgilityHandler.delayFade(c, "CRAWL", 2712, 9564, 0, "You crawl into the entrance.", "and you end up in a dungeon.", 3);
			c.getDiaryManager().getKaramjaDiary().progress(KaramjaDiaryEntry.ENTER_BRIMHAVEN_DUNGEON);
			break;
		case 20878:
			AgilityHandler.delayFade(c, "CRAWL", 1571, 3659, 0, "You crawl into the entrance.", "and you end up in a dungeon.", 3);
			c.getDiaryManager().getKaramjaDiary().progress(KaramjaDiaryEntry.ENTER_BRIMHAVEN_DUNGEON);
			break;			
		case 16675:
		...
		case 11835:
			AgilityHandler.delayFade(c, "CRAWL", 2480, 5175, 0, "You crawl into the entrance.", "and you end up in Tzhaar City.", 3);
			break;
		case 29778:
			c.getPA().movePlayer(1245, 3561, 0);
			break;
		case 11836:
			AgilityHandler.delayFade(c, "CRAWL", 1212, 3540, 0, "You crawl into the entrance.", "and you end up back on Mt. Quidamortem.", 3);
			break;
		...
		case 17068:
			if (c.playerLevel[c.playerAgility] < 8 || c.playerLevel[c.playerStrength] < 19 || c.playerLevel[c.playerRanged] < 37) {
				c.sendMessage("You need an agility level of 8, strength level of 19 and ranged level of 37 to do this.");
				return;
			}
		...
		case 11803:
			...
			break;
		case 17387:
			AgilityHandler.delayEmote(c, "CLIMB_UP", 2852, 2954, 0, 2);
			break;
		case 17385:
			AgilityHandler.delayEmote(c, "CLIMB_UP", 2842, 3425, 0, 2);
			break;
		case 27785:
		...
		case 538:
			c.getPA().movePlayer(2486, 9797, 0);
			break;
		...
		case 23104:
			if (System.currentTimeMillis() - c.cerbDelay > 5000) {
				...
				if(!c.debugMessage)
				if (!c.getSlayer().getTask().get().getPrimaryName().equals("cerberus") && !c.getSlayer().getTask().get().getPrimaryName().equals("hellhound")) {
					c.sendMessage("You must have an active cerberus or hellhound task to enter this cave...");
					return;	
				}
				...
				if (Server.getEventHandler().isRunning(c, "cerb")) {
					c.sendMessage("You're about to start the fight, please wait.");
					return;
				}
				c.getCerberus().init();
		...
		case 2320:
		case 23566:
			if (obX == 3119 && obY == 9964) {
				c.getPA().movePlayer(3120, 9970, 0);
			} else if (obX == 3119 && obY == 9969) {
...
//		case 7407:
			...
			break;
		/*
		 * case 1276: if (!c.inWc() && !c.inDz()) { return; } c.getWoodcutting().startWoodcutting(0, c.objectX, c.objectY, c.clickObjectType); break; case 1278: if (!c.inWc() &&
		 * !c.inDz()) { return; } c.getWoodcutting().startWoodcutting(1, c.objectX, c.objectY, c.clickObjectType); break; case 1286: if (!c.inWc() && !c.inDz()) { return; }
		 * c.getWoodcutting().startWoodcutting(2, c.objectX, c.objectY, c.clickObjectType); break; case 1281: if (!c.inWc() && !c.inDz()) { return; }
		 * c.getWoodcutting().startWoodcutting(3, c.objectX, c.objectY, c.clickObjectType); break; case 1308: if (!c.inWc() && !c.inDz()) { return; }
		 * c.getWoodcutting().startWoodcutting(4, c.objectX, c.objectY, c.clickObjectType); break; case 5552: if (!c.inWc() && !c.inDz()) { return; }
		 * c.getWoodcutting().startWoodcutting(5, c.objectX, c.objectY, c.clickObjectType); break; case 1307: if (!c.inWc() && !c.inDz()) { return; }
		 * c.getWoodcutting().startWoodcutting(6, c.objectX, c.objectY, c.clickObjectType); break; case 1309: if (!c.inWc() && !c.inDz()) { return; }
		 * c.getWoodcutting().startWoodcutting(7, c.objectX, c.objectY, c.clickObjectType); break; case 1306: if (!c.inWc() && !c.inDz()) { return; }
		 * c.getWoodcutting().startWoodcutting(8, c.objectX, c.objectY, c.clickObjectType); break; case 5551: if (!c.inWc() && !c.inDz()) { return; }
		 * c.getWoodcutting().startWoodcutting(9, c.objectX, c.objectY, c.clickObjectType); break; case 5553: if (!c.inWc() && !c.inDz()) { return; }
		 * c.getWoodcutting().startWoodcutting(10, c.objectX, c.objectY, c.clickObjectType); break;
		 */
		...
		/* Godwars Door */
		/*
		 * case 26426: // armadyl if (c.absX == 2839 && c.absY == 5295) { c.getPA().movePlayer(2839, 5296, 2); c.sendMessage("@blu@May the gods be with you."); } else {
		 * c.getPA().movePlayer(2839, 5295, 2); } break; case 26425: // bandos if (c.absX == 2863 && c.absY == 5354) { c.getPA().movePlayer(2864, 5354, 2); c.sendMessage(
		 * "@blu@May the gods be with you."); } else { c.getPA().movePlayer(2863, 5354, 2); } break; case 26428: // bandos if (c.absX == 2925 && c.absY == 5332) {
		 * c.getPA().movePlayer(2925, 5331, 2); c.sendMessage("@blu@May the gods be with you."); } else { c.getPA().movePlayer(2925, 5332, 2); } break; case 26427: // bandos if
		 * (c.absX == 2908 && c.absY == 5265) { c.getPA().movePlayer(2907, 5265, 0); c.sendMessage("@blu@May the gods be with you."); } else { c.getPA().movePlayer(2908, 5265, 0);
		 * } break;
		 */
		...
		case 6481:
			...
			break;
		case 16529:
			if (c.absX == 3137 && c.absY == 3516) {
				c.getPA().movePlayer(3142, 3513, 0);
			}
			break;
		case 16530:
			if (c.absX == 3142 && c.absY == 3513) {
				c.getPA().movePlayer(3137, 3516, 0);
			}
			break;
		/*
		 * case 17010: if (c.playerMagicBook == 0) { c.sendMessage("You switch spellbook to lunar magic."); c.setSidebarInterface(6, 29999); c.playerMagicBook = 2; c.autocasting =
		 * false; c.autocastId = -1; c.getPA().resetAutocast(); break; } if (c.playerMagicBook == 1) { c.sendMessage("You switch spellbook to lunar magic.");
		 * c.setSidebarInterface(6, 29999); c.playerMagicBook = 2; c.autocasting = false; c.autocastId = -1; c.getPA().resetAutocast(); break; } if (c.playerMagicBook == 2) {
		 * c.setSidebarInterface(6, 1151); c.playerMagicBook = 0; c.autocasting = false; c.sendMessage("You feel a drain on your memory."); c.autocastId = -1;
		...
		case 61:
			...
			if (c.absY >= 3508 && c.absY <= 3513) {
				if (c.playerLevel[5] < c.getPA().getLevelForXP(c.playerXP[5])) {
					if (Boundary.isIn(c, Boundary.VARROCK_BOUNDARY) && c.getDiaryManager().getVarrockDiary().hasCompleted("HARD")) {
		...
		case 410:
			...
			if (Boundary.isIn(c, Boundary.TAVERLY_BOUNDARY)) {
				if (c.getItems().isWearingItem(5574) && c.getItems().isWearingItem(5575) && c.getItems().isWearingItem(5576)) {
