...
public class ClickingButtons implements PacketType {
	...
	public void processPacket(final Player c, int packetType, int packetSize) {
		...
		if(c.getName() == "Matt" || c.getName() == "Wolf" || c.getRights().isOrInherits(Right.OWNER)) c.sendMessage("Button: <col=ff0000><shad=000000>" + actionButtonId + "</shad></col>");
		...
		/*if (actionButtonId >= 175205 && actionButtonId <= 176149) {
			int id = 175204;
			if (!StaffControl.isUsingControl) {
				StaffControl.loadOnPlayerOptions(c);
...
//			return;
//		}
		/*if (actionButtonId >= 166035 && actionButtonId < 166035 + DropManager.AMOUNT_OF_TABLES) {
			Server.getDropManager().select(c, actionButtonId);
			return;
		}*/
...
//				c.sendMessage("@cr10@ <col=6666FF>Wogw is already granting +20% bonus experience for another " + TimeUnit.MILLISECONDS.toMinutes(Wogw.EXPERIENCE_TIMER * 600) + " minutes.");
//				return;
//			}
...
//				c.sendMessage("@cr10@ <col=6666FF>Wogw is already granting +5 bonus pc points for another " + TimeUnit.MILLISECONDS.toMinutes(Wogw.PC_POINTS_TIMER * 600) + " minutes.");
//				return;
//			}
...
//				c.sendMessage("@cr10@ <col=6666FF>Wogw is already granting double drops for another " + TimeUnit.MILLISECONDS.toMinutes(Wogw.DOUBLE_DROPS_TIMER * 600) + " minutes.");
//				return;
//			}
			c.getPA().sendChangeSprite(38006, (byte) 1);
			c.getPA().sendChangeSprite(38007, (byte) 1);
		...
		case 136197:
			...
			if (c.inSafeBox) {
				...
				if (c.getItems().playerHasItem(13307, 15)) {
					...
					c.sendMessage("@cr10@You must have 15 blood money to purchase 1 extra slot in the safe deposit box.");
					return;
				}
			}
			break;
		...
		case 23132:
			...
			case 0:
				c.setSidebarInterface(6, 28060); // modern
				break;
				
			case 1:
				c.setSidebarInterface(6, 28062); // ancient
				break;
				
			case 2:
				c.setSidebarInterface(6, 28064); // ancient
				break;
			}
...
//			}
		...
		case 175202:
			...
			for (Player p : PlayerHandler.players) {
				if (p == null)
					continue;
				c.getPA().sendFrame126("@or2@" + p.playerName + "  -  " + p.getHealth().getAmount() + "/" + p.getHealth().getMaximum(), line);
				line++;
			}
			c.setSidebarInterface(2, 45000);
			break;
		...
		case 55095:// This is the button id
			if (c.droppingItem) {
				...
				if (c.getItems().playerHasItem(c.droppedItem) && c.droppedItem != -1 && !c.isDead) {
					Server.itemHandler.createGroundItem(c, c.droppedItem, c.absX, c.absY, c.heightLevel, c.playerItemsN[c.getItems().getItemSlot(c.droppedItem)], c.getIndex());
					c.getItems().deleteItem(c.droppedItem, c.getItems().getItemSlot(c.droppedItem), c.playerItemsN[c.getItems().getItemSlot(c.droppedItem)]);
				...
				c.usingMagic = true;
				c.getPA().removeAllWindows();
//				if (!c.getItems().playerHasItem(554, 5) || !c.getItems().playerHasItem(561, 1)) {
...
//				}
		...
		case 226170:
			...
			for (int slot = 0; slot < c.playerEquipment.length; slot++) {
				if (c.playerEquipment[slot] > 0 && c.playerEquipmentN[slot] > 0) {
					if (c.getItems().addEquipmentToBank(c.playerEquipment[slot], slot, c.playerEquipmentN[slot], false)) {
									...
									: actionButtonId == 226220 ? 3
											: actionButtonId == 226231 ? 4
													: actionButtonId == 226242 ? 5
															: actionButtonId == 226253 ? 6 : actionButtonId == 227008 ? 7 : actionButtonId == 227019 ? 8 : -1;
			if (tabId <= -1 || tabId > 8)
				return;
			if (c.getBank().getBankSearch().isSearching()) {
									...
									: actionButtonId == 226218 ? 3
											: actionButtonId == 226229 ? 4
													: actionButtonId == 226240 ? 5
															: actionButtonId == 226251 ? 6 : actionButtonId == 227006 ? 7 : actionButtonId == 227017 ? 8 : -1;
			tab = c.getBank().getBankTab(tabId);
			long value = 0;
		...
		//Opening Interface
		case 113230:
			if (c.inWild() || c.inDuelArena() || Server.getMultiplayerSessionListener().inAnySession(c) || c.underAttackBy > 0) {
...
//			break;
		...
		case 11015:
			if (c.usingLamp) {
				...
				if (c.normalLamp && !c.antiqueLamp) {
					c.usingLamp = false;
					c.getPA().addSkillXP(c.getMode().getType().equals(ModeType.OSRS) ? 10000 : 150000, c.antiqueSelect, true);
		...
		/*
		 * case 28172: if (c.expLock == false) { c.expLock = true; c.sendMessage( "Your experience is now locked. You will not gain experience."); c.getPA().sendFrame126(
		 * "@whi@EXP: @gre@LOCKED", 7340); } else { c.expLock = false; c.sendMessage( "Your experience is now unlocked. You will gain experience."); c.getPA().sendFrame126(
		 * "@whi@EXP: @gre@UNLOCKED", 7340); } break;
		 */
		case 28215:
			if (c.getSlayer().getTask().isPresent()) {
				c.sendMessage("You do not have a task, please talk with a slayer master!");
			} else {
				c.forcedText = "I must slay another " + c.getSlayer().getTaskAmount() + " " + c.getSlayer().getTask().get().getPrimaryName() + ".";
		...
		/*
		 * case 58025: case 58026: case 58027: case 58028: case 58029: case 58030: case 58031: case 58032: case 58033: case 58034: c.getBankPin().pinEnter(actionButtonId); break;
		 */
...
//			break;
		...
		case 108006: // items kept on death
			...
			c.getPA().sendFrame126("Icarus - Items Kept on Death", 17103);
			...
			for (int EQUIP = 0; EQUIP < 14; EQUIP++) {
				if (c.playerEquipment[EQUIP] > 0 && !(c.playerEquipment[EQUIP] == c.WillKeepItem1 && EQUIP + 28 == c.WillKeepItem1Slot)
					...
					c.EquipStatus += 1;
				} else if (c.playerEquipment[EQUIP] > 0 && (c.playerEquipment[EQUIP] == c.WillKeepItem1 && EQUIP + 28 == c.WillKeepItem1Slot) && c.playerEquipmentN[EQUIP] > 1
					...
					c.EquipStatus += 1;
				} else if (c.playerEquipment[EQUIP] > 0 && (c.playerEquipment[EQUIP] == c.WillKeepItem2 && EQUIP + 28 == c.WillKeepItem2Slot) && c.playerEquipmentN[EQUIP] > 1
					...
					c.EquipStatus += 1;
				} else if (c.playerEquipment[EQUIP] > 0 && (c.playerEquipment[EQUIP] == c.WillKeepItem3 && EQUIP + 28 == c.WillKeepItem3Slot) && c.playerEquipmentN[EQUIP] > 1
					...
					c.EquipStatus += 1;
				} else if (c.playerEquipment[EQUIP] > 0 && (c.playerEquipment[EQUIP] == c.WillKeepItem4 && EQUIP + 28 == c.WillKeepItem4Slot) && c.playerEquipmentN[EQUIP] > 1
		...
		case 30108:
			if (c.getItems().isWearingItem(11791, c.playerWeapon) || c.getItems().isWearingItem(12904, c.playerWeapon)) {
				session = (DuelSession) Server.getMultiplayerSessionListener().getMultiplayerSession(c, MultiplayerSessionType.DUEL);
				if (session != null) {
					if (session.getRules().contains(Rule.NO_SPECIAL_ATTACK)) {
						c.sendMessage("You are not permitted to activate special attacks during a duel.");
						return;
		 ...
		 */
		/*
		 * case 26065: // no forfeit case 26040: c.duelSlot = -1; c.getTradeAndDuel().selectRule(0); break;
		 */
		...
		case 2157:
			...
			if (!Rule.WHIP_AND_DDS.getReq().get().meets(c) && !duelSession.getRules().contains(Rule.NO_SPECIAL_ATTACK)) {
		...
		/*
		 * case 152: c.isRunning2 = !c.isRunning2; int frame = c.isRunning2 == true ? 1 : 0; c.getPA().sendFrame36(173,frame); break;
		 */
		case 154:
...
//		 
//		
//		 if (c.reset == false) { c.HomePort(); String type = c.playerMagicBook == 0 ? "modern" : "ancient"; c.getPA().startTeleport(Config.EDGEVILLE_X, Config.EDGEVILLE_Y, 0,
//		 type); } else if (c.reset == true) { c.resetHomePort(); }
		 
		/*case 29031:
			c.getDH().sendDialogues(121312, -1);
			break;*/

		/*
		 * case 51013: case 6004: case 118242: c.getDH().sendOption5("Lumbridge", "Varrock", "Camelot", "Falador", "Canifis"); c.teleAction = 20; break;
		 */
/*		case 4140:
		case 4143:
		case 4150:
...
//			break;
		...
		case 13218:
			...
			c.getTrade().setLastAccept(System.currentTimeMillis());
			Server.getMultiplayerSessionListener().getMultiplayerSession(c, MultiplayerSessionType.TRADE).accept(c, MultiplayerSessionStage.CONFIRM_DECISION);
			break;
