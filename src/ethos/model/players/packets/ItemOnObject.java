	...
	/*	case 26782: //Fountain of Rune
		...
		case 26782: //Glory recharging
			if (itemId == 1704) {
					int glory_0 = 1704;
					int glory_6 = 19707;
					int glory_eternal = 11978;
					int amount = c.getItems().getItemAmount(glory_0);
					for(int i=0; i<amount; i++) {
						int random = Misc.random(500);
						c.getItems().deleteItem(glory_0, 1);
						c.startAnimation(811);
						if(random==0) {
							c.getItems().addItem(glory_6, 1);
							PlayerHandler.executeGlobalMessage("<col=FF0000>[Fountain of Rune] @cr19@ "+ c.getName() + " has just received an Eternal Glory!");
							c.getDH().sendItemStatement("You find an Eternal Glory!", glory_eternal);
							break;
						} else {
							c.getItems().addItem(glory_eternal, 1);							
						}
