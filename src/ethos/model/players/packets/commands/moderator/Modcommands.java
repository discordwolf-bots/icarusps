...
public class Modcommands extends Command {
	...
	public void execute(Player c, String input) {
		...
		int counter = 8144;
		c.getPA().sendFrame126("@dre@Icarus Commands", counter++);
		c.getPA().sendFrame126("", counter++);
		counter++; // 8146 is already being used
		c.getPA().sendFrame126("@dre@Moderators Only", counter++);
		counter = sendCommands(c, "moderator", counter);
		c.getPA().sendFrame126("", counter++);
...
//		counter = sendCommands(c, "donator", counter);
	...
	public Optional<String> getDescription() {
		return Optional.of("Shows a list of Moderator commands");
	}
