...
public class TwoOptions {
	...
	public static void handleOption1(Player c) {
		...
		if (c.teleAction == 1337) {
			c.getPA().startTeleport(1504, 3419, 0, "modern");
		}
		if (c.dialogueAction == 125) {
			if (c.getItems().playerHasItem(8851, 200) || SkillcapePerks.ATTACK.isWearing(c) || SkillcapePerks.isWearingMaxCape(c)) {
...
//		}
	...
	public static void handleOption2(Player c) {
		...
		case 901:
			...
			break;
		case 784:
			c.getDH().sendItemStatement("Your item is worth "+ Misc.format((int) c.wellItemPrice) +" gp.", c.wellItem);
			break;
		case 786:
			c.getPA().closeAllWindows();
			break;
		...
		if (c.teleAction == 1337) {
			c.getPA().startTeleport(1721, 3464, 0, "modern");
		}
