...
public class PlayerAssistant {
	...
	public void destroyItem() {
		...
		if (c.getItems().playerHasItem(itemId) && itemId!= -1 && c.playerItemsN[c.getItems().getItemSlot(itemId)] > 0) {
    ...
    private boolean checkRetributionReqsSingle(int i) {
        ...
        int combatDif1 = c.getCombat().getCombatDifference(c.combatLevel, PlayerHandler.getPlayers().get(i).combatLevel);
        if (PlayerHandler.getPlayers().get(i) == null || i != c.getIndex() || !PlayerHandler.getPlayers().get(i).inWild() || combatDif1 > c.wildLevel || combatDif1 > PlayerHandler.getPlayers().get(i).wildLevel) {
            return false;
        }
        if (Config.SINGLE_AND_MULTI_ZONES) {
            if (!PlayerHandler.getPlayers().get(i).inMulti()) {
                if (PlayerHandler.getPlayers().get(i).underAttackBy == c.getIndex() || PlayerHandler.getPlayers().get(i).underAttackBy <= 0 || PlayerHandler.getPlayers().get(i).getIndex() == c.underAttackBy || c.underAttackBy <= 0) {
                    return false;
                }
            }
        }
	...
	public void destroyInterface(String config) {// Destroy item created by Remco
		...
		String[][] info = { // The info the dialogue gives
				{ "Are you sure you want to "+config+" this item?", "14174" }, { "Yes.", "14175" }, { "No.", "14176" }, { "", "14177" },
				{ "If you wish to remove this confirmation,", "14182" }, { "simply type ::toggledrop.", "14183" }, { itemName, "14184" } };
		sendFrame34(itemId, 0, 14171, 1);
		for (String[] anInfo : info) sendFrame126(anInfo[0], Integer.parseInt(anInfo[1]));
		sendFrame164(14170);
	}
	...
	public void assembleSlayerHelmet() {
		...
		if (c.getItems().playerHasItem(4166) && c.getItems().playerHasItem(4168) && c.getItems().playerHasItem(4164) && c.getItems().playerHasItem(4551)
				&& c.getItems().playerHasItem(8901)) {
	...
	/**
	 * If the player is using melee and is standing diagonal from the opponent, then move towards opponent.
	 */
	...
	public void movePlayerDuel(int x, int y, int h) {
		...
		if (Objects.nonNull(session) && session.getStage().getStage() == MultiplayerSessionStage.FURTHER_INTERATION && Boundary.isIn(c, Boundary.DUEL_ARENA)) {
			return;
		}
		c.resetWalkingQueue();
		c.teleportToX = x;
...
public void sendFrame107() {
	 ...
	 * Creating projectile
	 **/
	private void createProjectile(int x, int y, int offX, int offY, int angle, int speed, int gfxMoving, int startHeight, int endHeight, int lockon, int time) {
		// synchronized(c) {
		if (c.getOutStream() != null && c != null) {
			c.getOutStream().createFrame(85);
	...
	public void createPlayersProjectile(int x, int y, int offX, int offY, int angle, int speed, int gfxMoving, int startHeight, int endHeight, int lockon, int time, int delay) {
		if (delay <= 0) {
	...
	public void useChargeSkills() {
		int[][] skillsNeck = {{11968, 11970, 5}, {11970, 11105, 4}, {11105, 11107, 3}, {11107, 11109, 2}, {11109, 11111, 1}, {11111, 11113, 0}};
	...
	public void useCharge() {
		...
		for (int[] aGlory : glory) {
			if (c.itemUsing==aGlory[0]) {
				if (c.isOperate) {
					c.getItems().deleteItem(aGlory[0], 1);
					c.getItems().addItem(aGlory[1], 1);
				}
				if (aGlory[2]>1) {
					c.sendMessage("Your amulet has "+aGlory[2]+" charges left.");
				} else {
					c.sendMessage("Your amulet has "+aGlory[2]+" charge left.");
				}
			}
		}
		//c.getItems().updateSlot(c.playerAmulet);
	...
	public void alchemy(int itemId, String alch) {
		...
		case "low":
			if (System.currentTimeMillis() - c.alchDelay > 1000) {
				...
				int current = c.getItems().getItemCount(995);
				if(current+amount > Config.MAX_STACK) {
					//c.sendMessage("Max cash reached!");
				}
				//c.sendMessage("Amount " + amount);
				c.getItems().addItem(995, amount);
				c.startAnimation(MagicData.MAGIC_SPELLS[49][2]);
				c.gfx100(MagicData.MAGIC_SPELLS[49][3]);
				c.alchDelay = System.currentTimeMillis();
				sendFrame106(6);
				addSkillXP(MagicData.MAGIC_SPELLS[49][7] * (c.getMode().getType().equals(ModeType.OSRS) ? 1 : 125), 6, true);
				refreshSkill(6);
			}
			break;
		...
		case "high":
			if (System.currentTimeMillis() - c.alchDelay > 2000) {
				...
				sendFrame106(6);
				addSkillXP(MagicData.MAGIC_SPELLS[50][7] * (c.getMode().getType().equals(ModeType.OSRS) ? 1 : 125), 6, true);
				refreshSkill(6);
			}
			break;
			}
	...
	public void giveLife() {
		...
		if (!c.inDuelArena() && !Boundary.isIn(c, Boundary.DUEL_ARENA) 
				&& !Boundary.isIn(c, Boundary.SAFEPKSAFE) 
				&& !Boundary.isIn(c, Boundary.FIGHT_CAVE)
				&& !Boundary.isIn(c, Boundary.LIGHTHOUSE) 
				&& !Boundary.isIn(c, PestControl.GAME_BOUNDARY)
				&& !c.inSafemode() 
				&& !Boundary.isIn(c, Boundary.INFERNO)
				&& !Boundary.isIn(c, Zulrah.BOUNDARY)
				&& !Boundary.isIn(c, Boundary.CERBERUS_BOSSROOMS) 
				&& !Boundary.isIn(c, Boundary.SKOTIZO_BOSSROOM) 
				&& !Boundary.isIn(c, Boundary.RFD) 
				&& !Boundary.isWithRaids(c)
				&& Lowpkarena.getState(c) == null && Highpkarena.getState(c) == null) {
			...
			//If any items are on this list, delete them
				...
				/*
				 * } else if (c.inPits) { Server.fightPits.removePlayerFromPits(c.playerId); c.pitsStatus = 1;
				 */
		} else if (Boundary.isIn(c, Boundary.PEST_CONTROL_AREA)) {
			c.getPA().movePlayer(2657, 2639, 0);
		} else if (Boundary.isWithRaids(c)) {
			c.getRaids().teleportToStart();
		}
		if (Boundary.isIn(c, PestControl.GAME_BOUNDARY)) {
			...
			c.talkingNpc = 2040;
			c.getDH().sendNpcChat("It looks like Zulrah beat you.", "I'll give you back your items for 500,000GP.", "Talk to me when you're ready.");
		} else if (Boundary.isIn(c, Boundary.CERBERUS_BOSSROOMS)) {
			c.getPA().movePlayer(1309, 1250, 0);
			c.getCerberusLostItems().store();
			c.talkingNpc = 5870;
			c.getDH().sendNpcChat("It looks like Cerberus beat you.", "I'll give you back your items for 500,000GP.", "Talk to me when you're ready.");
		} else if (Boundary.isIn(c, Boundary.SKOTIZO_BOSSROOM)) {
			c.getPA().movePlayer(1665, 10045, 0);
			c.getSkotizoLostItems().store();
			c.talkingNpc = 7283;
			c.getDH().sendNpcChat("It looks like Skotizo beat you.", "I'll give you back your items for 500,000GP.", "Talk to me when you're ready.");
		...
		} else if (Boundary.isWithRaids(c) && c.getRaids().getRaidInstance() != null) {
			c.getRaids().teleportToStart();
	...
	public void followNpc() {
		...
		boolean projectile = c.usingOtherRangeWeapons || c.usingBow || c.usingMagic || c.autocasting || c.getCombat().usingCrystalBow();
		if (!projectile || projectile && (PathChecker.isProjectilePathClear(c.absX, c.absY, c.heightLevel, otherX, otherY) 
			...
			if (c.playerEquipment[c.playerWeapon] == 11907 || c.playerEquipment[c.playerWeapon] == 12899 && bowDistance && !sameSpot) {
				return;
			}
		...
		final boolean inWater = npc.npcType == 2042 || npc.npcType == 2043 || npc.npcType == 2044 || npc.npcType == 1739 || npc.npcType == 1740 || npc.npcType == 1741 || npc.npcType == 1742;
	...
	/*
	 * public void Obelisks(int id) { if (!c.getItems().playerHasItem(id)) { c.getItems().addItem(id, 1); } }
	 */
	...
	public boolean addSkillXP(int amount, int skill, boolean dropExperience) {
		...
		if (Boundary.isIn(c, Boundary.DONATOR_ZONE) && c.getRights().isOrInherits(Right.DONATOR) && !DoubleExperience.isDoubleExperience(c)) {
			amount *= Config.SERVER_EXP_BONUS_WEEKEND_BOOSTED;

		//If within thedonator zone, VIP accounts get bonus xp while bonus weekend is on
		} else if (Boundary.isIn(c, Boundary.DONATOR_ZONE) && c.getRights().isOrInherits(Right.DONATOR) && DoubleExperience.isDoubleExperience(c)) {
			amount *= Config.SERVER_EXP_BONUS_WEEKEND_BOOSTED + Config.SERVER_EXP_BONUS;
			
		//If bonus weekend is enabled but no vote bonus xp is active, you get reg bonus (20%)
		} else if (DoubleExperience.isDoubleExperience(c) && c.bonusXpTime == 0) {
			amount *= Config.SERVER_EXP_BONUS_WEEKEND;
			
		//If bonus weekend is enabled and vote bonus xp is active, you get additional bonus (35%)
		} else if (DoubleExperience.isDoubleExperience(c) && c.bonusXpTime > 0) {
			amount *= Config.SERVER_EXP_BONUS_WEEKEND_BOOSTED;
			
		//If bonus weekend is disabled and vote bonus xp is active, you get reg vote bonus (15%)
		} else if (!DoubleExperience.isDoubleExperience(c) && c.bonusXpTime > 0) {
			amount *= Config.SERVER_EXP_BONUS_BOOSTED;
			
		//If wogw bonus xp is enabled and vote bonus is active, you get additional bonus (35%)
		} else if (Config.BONUS_XP_WOGW && c.bonusXpTime > 0) {
			amount *= Config.SERVER_EXP_BONUS_BOOSTED;
		//If wogw bonus xp is enabled and bonus weekend is active, you get additional bonus (35%)
	...
	public boolean checkForFlags() {
		int[][] itemsToCheck = { { 995, 100000000 }, { 35, 5 }, { 667, 5 }, { 2402, 5 }, { 746, 5 }, { 4151, 150 }, { 565, 100000 }, { 560, 100000 }, { 555, 300000 },
				{ 11235, 10 } };
	...
	public void logStuck() {
		...
		try {
			...
			bw.write("[" + c.playerName + "]: " + "[" + c.connectedFrom + "] " + ": " + " X: " + c.getX() + " Y: " + c.getY() + "]");
			bw.newLine();
			bw.flush();
	...
	public static boolean ringOfCharosTeleport(final Player player) {
		...
		player.getPA().startTeleport(x, y, z, "modern");
		return true;
	}
	...
	public void useOperate(int itemId) {
		...
		case 13136:
			...
			c.getPA().spellTeleport(3426, 2927, 0);
			break;
		...
		case 13111:
			if (c.getRechargeItems().useItem(itemId)) {
				c.getPA().spellTeleport(3236, 3946, 0);
			}
			break;
		case 10507:
		...
		case 11283:
		case 11284:
			if (Boundary.isIn(c, Zulrah.BOUNDARY) || Boundary.isIn(c, Boundary.CERBERUS_BOSSROOMS) || Boundary.isIn(c, Boundary.SKOTIZO_BOSSROOM)) {
				return;
			}
		...
		case 2564:
		case 2566:
			c.getPA().spellTeleport(3304, 3130, 0);
			break;
	...
	/*
	 * public void fixAllBarrows() { int totalCost = 0; int cashAmount = c.getItems().getItemAmount(995); for (int j = 0; j < c.playerItems.length; j++) { boolean breakOut = false;
	 * for (int i = 0; i < c.getItems().brokenBarrows.length; i++) { if (c.playerItems[j]-1 == c.getItems().brokenBarrows[i][1]) { if (totalCost + 80000 > cashAmount) { breakOut =
	 * true; c.sendMessage("You have run out of money."); break; } else { totalCost += 80000; } c.playerItems[j] = c.getItems().brokenBarrows[i][0]+1; } } if (breakOut) break; } if
	 * (totalCost > 0) c.getItems().deleteItem(995, c.getItems().getItemSlot(995), totalCost); }
	 */
	...
	/**
	 * Exchanges all items in the player owners inventory to a specific to whatever the exchange specifies. Its up to the switch statement to make the conversion.
	 * 
	...
	public static void noteItems(Player player, int item) {
		...
		if (definition == null || notedDefinition == null || !notedDefinition.getName().contains(definition.getName())) {
	...
	public static void decantResource(Player player, int item) {
		...
		if (!player.getRights().isOrInherits(Right.CONTRIBUTOR)) {
			player.getDH().sendStatement("You note most of your " + definition.getName() + " at the cost of " + cost + " resources.");
		} else {
			player.getDH().sendStatement("You note all your " + definition.getName() + ".");
		}
		player.nextChat = -1;
...
//			System.out.println("sendGameTimer: null user or disconnected");
//			return;
//		}
//		Stream stream = c.getOutStream();
//		if (stream == null) {
//			System.out.println("sendGameTimer: null stream");
//			return;
//		}
//		//System.out.println(duration);
//		int seconds = (int) Long.min(unitOfTime.toSeconds(duration), 65535);
//		System.out.println("sendGameTimer: seconds" + seconds);
...
//		c.flushOutStream();
	...
	public void sendExperienceDrop(boolean increase, long amount, int... skills) {
		...
		if (!illegalSkills.isEmpty()) {
			skills = ArrayUtils.removeAll(skills, ArrayUtils.toPrimitive(illegalSkills.toArray(new Integer[illegalSkills.size()])));
		}
