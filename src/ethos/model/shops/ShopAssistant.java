...
public class ShopAssistant {
	...
	public void buyFromShopPrice(int removeId, int removeSlot) {
		...
		if (c.myShopId == 9) {
			c.sendMessage(ItemAssistant.getItemName(removeId) + ": currently costs [ @blu@" + getSpecialItemValue(removeId) + " @bla@ ] Donator Credits.");
			return;
		}
		if (c.myShopId == 18) {
	...
	public void handleOtherShop(int itemID, int amount) {
		...
		/*} else if (c.myShopId == 82) { //Disabled
			if (c.raidPoints >= getSpecialItemValue(itemID) * amount) {
				if (c.getItems().freeSlots() > 0) {
					c.raidPoints -= getSpecialItemValue(itemID) * amount;
					c.getPA().loadQuests();
					c.getPA().sendFrame126(ShopHandler.ShopName[118] + " - " + c.raidPoints + " Assault Points", 64003);
					c.getItems().addItem(itemID, amount);
					c.getItems().resetItems(3823);
				}
			} else {
