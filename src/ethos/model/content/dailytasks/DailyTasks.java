	...
	public static void assignTask(Player player) {
		if (player.dailyEnabled) {
			...
			if(player.currentTask != null) {
				player.sendMessage("@blu@@cr10@ You already have a daily task. Type '::dailytask' to check your task!");
				return;
			}
			
			if (player.dailyTaskDate == getTodayDate() && player.currentTask == null && player.completedDailyTask == true) {
				player.sendMessage("@blu@@cr10@ You already completed your daily task");
				return;
			} else {
				if (player.completedDailyTask == false && player.dailyTaskDate != getTodayDate() && player.currentTask == null) {
					...
					player.sendMessage("@blu@@cr10@ New Daily Task: " + player.currentTask.message);
				}
			}
			
		} else
			player.sendMessage("@blu@@cr10@ Go speak to the daily task master to enable daily tasks!");
	}
