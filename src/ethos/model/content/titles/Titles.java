...
public class Titles {
	...
	public void display() {
		...
		view(Title.NONE);
		player.getPA().showInterface(29665);
	}
...
//		}
	...
	private void view(Title title) {
		...
		player.getPA().sendString(description, 33011);
		if (title.equals(Title.CUSTOM)) {
			player.getPA().sendString(currentTitle, 33036);
			player.getPA().sendFrame171(1, 33030);
			player.getPA().sendFrame171(0, 33035);
		} else {
			player.getPA().sendString(title.getName(), 33031);
			player.getPA().sendFrame171(0, 33030);
			player.getPA().sendFrame171(1, 33035);
		}
		player.getPA().sendString(isPurchased(title) ? "Select" : "Purchase", 33012);
		player.getPA().sendString(title.getCost() == 0 ? "FREE" : Misc.getValueRepresentation(title.getCost()), 33015);
		player.getPA().sendChangeSprite(33017, title.getCurrency().getSpriteIndex());
	}
