...
public class PetHandler {
	...
	/**
	 * A {@link Set} of {@link Pets} that represent non-playable characters that a player entity can drop and interact with.
	 */
	...
	public static enum Pets {
		...
		CHAOS(11995, 4444, "Chaos Elemental", 500, "first"), 
	...
	public static void spawn(Player player, Pets pet, boolean ignore, boolean ignoreAll) {
		...
		if (pet.itemId == 12840 && !ignore) {
			if (player.getHolidayStages().getStage("Halloween") < 6 && player.getRights().isOrInherits(Right.OWNER)) {
			...
			switch (randomDeath) {
			case 0:
				Server.npcHandler.spawnNpc3(player, 5568, player.absX + offsetX, player.absY + offsetY, player.heightLevel, 0, 0, 0, 0, 0,
						true, false, true);
				break;
				
			case 1:				
				Server.npcHandler.spawnNpc3(player, 5569, player.absX + offsetX, player.absY + offsetY, player.heightLevel, 0, 0, 0, 0, 0,
					true, false, true);
				break;
				
			case 2:				
				Server.npcHandler.spawnNpc3(player, 5570, player.absX + offsetX, player.absY + offsetY, player.heightLevel, 0, 0, 0, 0, 0,
					true, false, true);
				break;
				
			case 3:				
				Server.npcHandler.spawnNpc3(player, 5571, player.absX + offsetX, player.absY + offsetY, player.heightLevel, 0, 0, 0, 0, 0,
					true, false, true);
				break;
			}
		} else {
			...
			player.summonId = pet.itemId;
			PlayerSave.saveGame(player);
			Server.npcHandler.spawnNpc3(player, pet.npcId, player.absX + offsetX, player.absY + offsetY, player.heightLevel, 0, 0, 0, 0, 0, true, false, true);
			if (!ignore) {
			}
		}
	}
...
//				NPCHandler.npcs[player.npcClickIndex].requestTransform(randomGold == 15 ? 6721 : npcId == 6721 ? 6718 : npcId == 6720 ? npcId - 2 : npcId + 1);
//				break;
			}
		}
	}
	
	/*public static void recolor(Player player, int npcId, int itemId) {
		Pets pets = forNpc(npcId);
		if (npcId < 1) {
			return;
		}
		if (pets != null) {
			...
			case 7439:
				switch (itemId) {
				case 438:
					NPCHandler.npcs[player.npcClickIndex].requestTransform(7440);
				break;
				}
			break;
