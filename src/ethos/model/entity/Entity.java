...
/**
 * Represents a game-world presence that exists among others of the same nature. The objective is to allow multiple entities to share common similarities and allow simple but
 * effective reference in doing so.
 * 
...
public abstract class Entity {
	...
	/**
	 * The visual alias or name of the entity that describes and separates this entity from others of the same nature.
	 */
	...
	/**
	 * Creates a new {@link Entity} object with a specified index value representing where in their respective array they reside. An {@link Entity} is an object that exists within
	 * the game-world. A {@link Player} or {@link NPC} are all examples of entities.
	 * 
	...
	/**
	 * The index value where the {@link Entity} resides along with other common counterparts.
	 * 
	...
	/**
	 * Sends some information to the Stream regarding a possible new hit on the entity.
	 * 
	...
	/**
	 * Used to append some amount of damage to the entity and inflict on their total amount of health. The method will also display a new hitmark on that entity.
	 * 
	 * @param damage the damage dealt
	 * @param hitmark the hitmark to show with the damage
	 */
	public abstract void appendDamage(int damage, Hitmark hitmark);

	/**
	 * Determines if the entity is susceptible to a status based on their nature. For example some players when wearing certain equipment are exempt from venom or poison status. In
	 * other situations, NPC's are susceptible to venom.
	 * 
	...
	/**
	 * When an entity dies it is paramount that we know who dealt the most damage to that entity so that we can determine who will receive the drop.
	 * 
	...
	/**
	 * The status of the entities health whether it's normal, poisoned, or some other nature.
	 * 
	 ...
	 * 
	 * @param killer the {@link Entity} killer
	 */
