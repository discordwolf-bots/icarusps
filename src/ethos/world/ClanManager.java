...
public class ClanManager {
	...
	public void create(Player paramClient) {
		...
		paramClient.sendMessage("<col=FF0000>You may change your clan settings by clicking the 'Clan Setup' button.");
	}
	...
	public Clan getHelpClan() {
		for (int i = 0; i < this.clans.size(); i++) {
			if (clans.get(i).getFounder().equalsIgnoreCase("Wolf")) {
				return clans.get(i);
			}
		}

		Clan localClan = read("Wolf");
		if (localClan != null) {
			clans.add(localClan);
			return localClan;
		}
		localClan = new Clan("Server", "Wolf");
		clans.add(localClan);
		localClan.save();
		return localClan;
	}
	...
	public void joinOnLogin(Player client) {
		...
		if (lastChannel != null && lastChannel.length() > 0) {
			client.sendMessage("Attempting to rejoin @blu@" + lastChannel + "@bla@...");
