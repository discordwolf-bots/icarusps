...
public class Clan {
	...
	public void addMember(Player player) {
		...
		player.getPA().sendString("Talking in: <col=FFFFFF>" + getTitle() + "</col>", 18139);
		player.getPA().sendString("Owner: <col=FFFFFF>" + Misc.formatPlayerName(getFounder()) + "</col>", 18140);
		player.sendMessage("Now talking in: @blu@" + getTitle() + "@bla@");
	...
	/**
	 * Returns whether or not the specified name is an admin.
	 * 
	 * @param name
	 * @return
	 */
	public boolean isAdmin(String name) {
		...
		if(p.getRights().isOrInherits(Right.ADMINISTRATOR))
			return true;
		return false;
	}
	
	/**
	 * Returns whether or not the specified name is a moderator
	 * 
	 * @param name
	 * @return
	 */
	public boolean isModerator(String name) {
		Player p = PlayerHandler.getPlayer(name);
		if(p.getRights().isOrInherits(Right.MODERATOR))
			return true;
		return false;
	}
